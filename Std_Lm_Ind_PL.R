#' A Function Provides Line Plots Comparing Raw and Indirect Standardized Outcomes Under Linear Model
#'
#' This function provides ggplot2 based line plots comparing raw and indirect standardized outcomes under linear model.
...
#' @keywords Line Plots, Indirect standardization under linear model
#' @export
#' @examples
#' Std_Lm_Ind_PL(data,"sex",c("BMI"),Std_by = "white", Ref = "median")

Std_Lm_Ind_PL <- function(Data, Outcome, Predictor, Std_by, Ref="median"){
  Data_Var <- names(Data)
  ...
  Test_Adj$Value <- apply(Test_Adj,1,function(X){
    return(Std_Lm_Ind(Data, Outcome, Predictor, Std_by, Group=X["Group"], Ref="median"))
  })
