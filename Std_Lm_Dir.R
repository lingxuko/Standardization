#' A Direct Standardization Function Under Linear Regression
#'
...
#' @keywords Direct standardization under linear model
#' @export
#' @examples
#' Std_Lm_Dir(data,"sex",c("BMI"),Std_by = "white", Group = 1,Ref = "median")

Std_Lm_Dir <- function(Data, Outcome, Predictor, Std_by, Group, Ref="median"){
Data_Var <- names(Data)
...
if(Ref=="median"){
  ...
  Numerator <- mean(predict(Mod, Data_ref), na.rm=TRUE)
