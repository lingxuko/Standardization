#' A Function Provides Line Plots Comparing Raw and Direct Standardized Outcomes Under Poisson Model
#'
#' This function provides ggplot2 based line plots comparing raw and direct standardized outcomes under Poisson model.
...
#' @keywords Line Plots, Direct standardization under Poisson model
#' @export
#' @examples
#' Std_Poi_Dir_PL(data,"sex",c("BMI"),Std_by = "white", Ref = "median")

Std_Poi_Dir_PL <- function(Data, Outcome, Predictor, Std_by, Ref="median"){
  Data_Var <- names(Data)
  ...
  Test_Adj$Value <- apply(Test_Adj,1,function(X){
    return(Std_Poi_Dir(Data, Outcome, Predictor, Std_by, Group=X["Group"], Ref="median"))
  })
