#' A Indirect Standardization Function Under Logistic Regression
#'
...
#' @keywords Indirect standardization under logistic model
#' @export
#' @examples
#' Std_Log_Ind(data,"sex",c("BMI"),Std_by = "white", Group = 1,Ref = "median")

Std_Log_Ind <- function(Data, Outcome, Predictor, Std_by, Group, Ref="median"){
  Data_Var <- names(Data)
  ...
  Data[,c(paste(Outcome))] <- as.factor(Data[,c(paste(Outcome))])
  ...
  if(Ref=="median"){
    Mod <- glm(paste(Model),data=Data,family = "binomial")
    Coef <- summary(Mod)$coefficients
    Coef2 <- c(median(Coef[(1:k),1]),Coef[(k+1):nrow(Coef),1])
    Numerator <- mean(predict(Mod, Data_ref2, type = "response"), na.rm=TRUE)
